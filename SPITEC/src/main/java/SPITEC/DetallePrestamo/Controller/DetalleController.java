package SPITEC.DetallePrestamo.Controller;

import SPITEC.DetallePrestamo.DTO.DetalleDTO;
import SPITEC.DetallePrestamo.Service.DetalleService;
...
public class DetalleController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<DetalleDTO>> guardar(@Valid @RequestBody DetalleDTO json) {
        try {
            DetalleDTO dto = service.nuevoDetalle(json);
            if (dto != null) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<DetalleDTO>>> obtenerTodo() {
        List<DetalleDTO> data = service.obtenerTodo();
        return ResponseEntity.ok(new ApiResponse<>(true, "Datos encontrados exitosamente", data));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<DetalleDTO>> buscarPorId(@PathVariable Long id) {
        DetalleDTO data = service.buscarPorId(id);
        if (data == null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<DetalleDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody DetalleDTO json) {
        DetalleDTO data = service.actualizar(id, json);
        if (data == null) {
