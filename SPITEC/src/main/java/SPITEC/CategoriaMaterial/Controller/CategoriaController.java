package SPITEC.CategoriaMaterial.Controller;

import SPITEC.CategoriaMaterial.DTO.CategoriaDTO;
import SPITEC.CategoriaMaterial.Service.CategoriaService;
...
public class CategoriaController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<CategoriaDTO>> guardar(@Valid @RequestBody CategoriaDTO json) {
        try {
            CategoriaDTO dto = service.nuevoCategoria(json);
            if (dto != null) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<CategoriaDTO>>> obtenerTodo() {
        List<CategoriaDTO> data = service.obtenerTodo();
        return ResponseEntity.ok(new ApiResponse<>(true, "Datos encontrados exitosamente", data));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<CategoriaDTO>> buscarPorId(@PathVariable Long id) {
        CategoriaDTO data = service.buscarPorId(id);
        if (data == null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<CategoriaDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody CategoriaDTO json) {
        CategoriaDTO data = service.actualizar(id, json);
        if (data == null) {
