package SPITEC.AreaMaterial.Controller;

import SPITEC.AreaMaterial.DTO.AreaMaterialDTO;
import SPITEC.AreaMaterial.Service.AreaMaterialService;
...
public class AreaMaterialController {
    private final AreaMaterialService service;

    public AreaMaterialController(AreaMaterialService service) {
        this.service = service;
    }

    @PostMapping
    public ResponseEntity<ApiResponse<AreaMaterialDTO>> guardar(@Valid @RequestBody AreaMaterialDTO json) {
        try {
            AreaMaterialDTO dto = service.nuevoAreaMaterial(json);
            if (dto != null) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<AreaMaterialDTO>>> obtenerTodo() {
        List<AreaMaterialDTO> data = service.obtenerTodo();
        return ResponseEntity.ok(new ApiResponse<>(true, "Datos encontrados exitosamente", data));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<AreaMaterialDTO>> buscarPorId(@PathVariable Long id) {
        AreaMaterialDTO data = service.buscarPorId(id);
        if (data == null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<AreaMaterialDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody AreaMaterialDTO json) {
        AreaMaterialDTO data = service.actualizar(id, json);
        if (data == null) {
