package SPITEC.Instituciones.Controller;

import SPITEC.Instituciones.DTO.InstitucionesDTO;
import SPITEC.Instituciones.Service.InstitucionesService;
...
public class InstitucionesController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<InstitucionesDTO>> guardar(@Valid @RequestBody InstitucionesDTO json) {
        try {
            InstitucionesDTO dto = service.nuevoInstitucion(json);
            if (dto != null) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<InstitucionesDTO>>> obtenerTodo() {
        List<InstitucionesDTO> data = service.obtenerTodo();
        return ResponseEntity.ok(new ApiResponse<>(true, "Datos encontrados exitosamente", data));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<InstitucionesDTO>> buscarPorId(@PathVariable Long id) {
        InstitucionesDTO data = service.buscarPorId(id);
        if (data == null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<InstitucionesDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody InstitucionesDTO json) {
        InstitucionesDTO data = service.actualizar(id, json);
        if (data == null) {
