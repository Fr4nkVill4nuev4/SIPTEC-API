package SPITEC.Permiso.Controller;

import SPITEC.Permiso.DTO.PermisosDTO;
import SPITEC.Permiso.Service.PermisosService;
...
public class PermisosController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<PermisosDTO>> guardar(@Valid @RequestBody PermisosDTO json) {
        try {
            PermisosDTO dto = service.nuevoPermiso(json);
            if (dto != null) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PermisosDTO>>> obtenerTodo() {
        List<PermisosDTO> data = service.obtenerTodo();
        return ResponseEntity.ok(new ApiResponse<>(true, "Datos encontrados exitosamente", data));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PermisosDTO>> buscarPorId(@PathVariable Long id) {
        PermisosDTO data = service.buscarPorId(id);
        if (data == null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PermisosDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody PermisosDTO json) {
        PermisosDTO data = service.actualizar(id, json);
        if (data == null) {
