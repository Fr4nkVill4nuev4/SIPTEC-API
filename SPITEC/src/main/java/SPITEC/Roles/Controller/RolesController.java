package SPITEC.Roles.Controller;

import SPITEC.Roles.DTO.RolesDTO;
import SPITEC.Roles.Service.RolesService;
...
public class RolesController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<RolesDTO>> guardar(@Valid @RequestBody RolesDTO json) {
        try {
            RolesDTO dto = service.nuevoRol(json);
            if (dto != null) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<RolesDTO>>> obtenerTodo() {
        List<RolesDTO> data = service.obtenerTodo();
        return ResponseEntity.ok(new ApiResponse<>(true, "Datos encontrados exitosamente", data));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<RolesDTO>> buscarPorId(@PathVariable Long id) {
        RolesDTO data = service.buscarPorId(id);
        if (data == null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<RolesDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody RolesDTO json) {
        RolesDTO data = service.actualizar(id, json);
        if (data == null) {
