package SPITEC.Materiales.Controller;

import SPITEC.Materiales.DTO.MaterialesDTO;
import SPITEC.Materiales.Service.MaterialesService;
...
public class MaterialesController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<MaterialesDTO>> guardar(@Valid @RequestBody MaterialesDTO json) {
        try {
            MaterialesDTO dto = service.nuevoMaterial(json);
            if (dto != null) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<MaterialesDTO>>> obtenerTodo() {
        List<MaterialesDTO> data = service.obtenerTodo();
        return ResponseEntity.ok(new ApiResponse<>(true, "Datos encontrados exitosamente", data));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<MaterialesDTO>> buscarPorId(@PathVariable Long id) {
        MaterialesDTO data = service.buscarPorId(id);
        if (data == null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<MaterialesDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody MaterialesDTO json) {
        MaterialesDTO data = service.actualizar(id, json);
        if (data == null) {
